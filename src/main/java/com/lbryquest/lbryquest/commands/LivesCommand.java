...
player.sendMessage(ChatColor.GREEN + "Lives are $" + (lbryQuest.BUYIN_AMOUNT*0.9) + " USD each in BTC. Most goes into Loot wallet which everyone searches for the treasure, once found that player will recive funds to their player wallet if no external wallet set and the world will reset for a new hunt. A little bit is set aside for further developent and hosting.");
	}
	if (balance == 0) {
	...
	if (lbryQuest.didVote(player.getName()) == 0) {
	player.sendMessage(ChatColor.YELLOW + "But to complete it you need to say buy to confirm so (/Lives 3 buy) will get you 3 lives for " + (3 * lbryQuest.totalLifeRate));
	} else if (lbryQuest.didVote(player.getName()) == 1) {
	player.sendMessage(ChatColor.YELLOW + "But to complete it you need to say buy to confirm so (/Lives 3 buy) will get you 3 lives for " + (3 * (lbryQuest.totalLifeRate*0.9)));
	}
		...
		if (args.length == 2) {
			if (args[1].equalsIgnoreCase("buy")) {
				...
				try {
					...
					player.sendMessage(ChatColor.GREEN + "You just got " + (lbryQuest.LIVES_PERBUYIN * livesAmount) + " lives!");
				...
				for (final Player onlinePlayer : Bukkit.getOnlinePlayers()) {
          			if (onlinePlayer.getName().equalsIgnoreCase(sendWho)) {
						...
						if (Integer.valueOf(lbryQuest.REDIS.get("LivesLeft" +player.getUniqueId().toString())) >= 2) {
							...
							String minusLives = Integer.toString((Integer.valueOf(lbryQuest.REDIS.get("LivesLeft" +player.getUniqueId().toString())) - livesAmount));
							String plusLives = Integer.toString((Integer.valueOf(lbryQuest.REDIS.get("LivesLeft" +Bukkit.getServer().getPlayer(sendWho).getUniqueId())) + livesAmount));
